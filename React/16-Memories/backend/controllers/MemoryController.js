...
const removeOldImage = (memory) => {
    fs.unlink(`public/${memory.src}`, (error) => {
        ...
        else console.log("Imagem excluída do servidor!");
    });
};
...
const createMemory = async (req, res) => {
    try {
        ...
        if (!title || !description) return res.status(400).json({ msg: "Por favor, preencha todos os campos!" });
...
const addComment = async (req, res) => {
    try {
        ...
        if (!name || !text) return res.status(400).json({ msg: "Por favor, preencha todos os campos!" });
        ...
        res.json({ updateMemory, msg: "Comentário adicionado!", memory });
    } catch (error) {
        res.status(500).send("Ocorreu um erro!");
    }
};
